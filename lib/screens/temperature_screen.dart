...
class _TemperatureScreenState extends State<TemperatureScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Center(
          child: StreamBuilder(
        stream: FirebaseDatabase.instance.ref("sensoresTemps").onValue,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (ventiladoresList.isEmpty) {
              return const Center(
                  child: Text('No tienes sensores, dale click al boton'));
            }
              ...
              builder: (context) {
                return AlertDialog(
                  ...
                  actions: [
                    ElevatedButton(
                      onPressed: () async {
                        await devicesProvider.createSensonTemperatureInDb(
